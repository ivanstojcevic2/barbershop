...
$("#contact_send").click(function () {
  ...
  if (
    $.trim(contact_name) === "" ||
    $.trim(contact_email) === "" ||
    $.trim(contact_subject) === "" ||
    $.trim(contact_message) === ""
  ) {
    $("#contact_status_message").html(
      "<div class = 'alert alert-danger'>One or more fields are empty!<div>"
    );
  } else {
...
function showTab(n) {
  ...
  if (n == x.length - 1) {
    document.getElementById("nextBtn").innerHTML = "Rezerviraj";
  } else {
...
function validateForm() {
  ...
  if (id_tab == "services_tab") {
    if (
      x[currentTab].querySelectorAll('input[type="checkbox"]:checked').length ==
      0
    ) {
  ...
  if (id_tab == "employees_tab") {
    if (
      x[currentTab].querySelectorAll('input[type="radio"]:checked').length == 0
    ) {
      x[currentTab].getElementsByClassName("alert")[0].style.display = "block";
      valid = false;
    } else {
      ...
      var selected_employee = $(
        "input[name='selected_employee']:checked"
      ).val();
